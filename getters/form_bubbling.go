...
// @submit.prevent that stops native submit and dispatches a bubbling CustomEvent "lamu-form-submit"
	...
	return func(ctx context.Context) (gomponents.Node, error) {
		...
		return gomponents.Group{
			...
			gomponents.Attr("@submit.prevent", fmt.Sprintf(
				`(function(evt){evt.preventDefault();var f=(evt.currentTarget&&evt.currentTarget.tagName==='FORM')?evt.currentTarget:(evt.target&&evt.target.closest&&evt.target.closest('form'));if(!f)return;f.dispatchEvent(new CustomEvent('lamu-form-submit',{bubbles:true,detail:{form:f,name:%s}}))})($event)`,
				nLit,
			)),
		}, nil
	}
...
// FormBubblingWithDataPostURL is [FormBubbling] plus data-lamu-post-url on the form (merges ?name= into postURLBase) so row-scoped POST targets work when many modal openers share one dialog id.
func FormBubblingWithDataPostURL(name Getter[string], postURLBase Getter[string]) Getter[gomponents.Node] {
	return func(ctx context.Context) (gomponents.Node, error) {
		...
		return gomponents.Group{bub, gomponents.Attr("data-lamu-post-url", u)}, nil
	}
}
