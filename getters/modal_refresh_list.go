...
// x-init handler that registers a single document listener for "lamu:modal-closed" (dispatched when
...
func ModalRefreshList(listURL, tableSelector Getter[string]) Getter[gomponents.Node] {
	return func(ctx context.Context) (gomponents.Node, error) {
		...
		script := fmt.Sprintf(
			"(function(){var u=%s;var s=%s;var k='lamu:mr:'+u+'|'+s;if(!window.__lamuModalRefreshKeys)window.__lamuModalRefreshKeys=new Set();if(window.__lamuModalRefreshKeys.has(k))return;window.__lamuModalRefreshKeys.add(k);document.addEventListener('lamu:modal-closed',function(){var url=u||window.location.pathname+window.location.search;fetch(url,{credentials:'same-origin',headers:{'HX-Request':'true','HX-Boosted':'true','Accept':'text/html'}}).then(function(r){if(!r.ok)return null;return r.text()}).then(function(h){if(h==null)return;var el=document.querySelector(s);if(!el)return;var doc=new DOMParser().parseFromString(h,'text/html');var fresh=doc.querySelector(s);if(fresh){el.innerHTML=fresh.innerHTML;return;}el.innerHTML=h;});});})()",
			uLit,
			sLit,
		)
