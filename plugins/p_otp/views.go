...
func redirectToRoute(w http.ResponseWriter, r *http.Request, routeKey string, args ...map[string]getters.Getter[any]) bool {
	...
	urlValue, err := getters.IfOr(lamu.RoutePath(routeKey, routeArgs), r.Context(), "")
	if err != nil || urlValue == "" {
		http.NotFound(w, r)
		return false
	}
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(fieldErrors) == 0 {
			...
			if err != nil {
				...
				verifyPath, _ := getters.IfOr(lamu.RoutePath("otp.OtpVerifyRoute", nil), r.Context(), "")
		...
		Entries: []registry.Pair[string, *views.View]{
			{
				...
				Value: lamu.GetPageView("otp.ForgotPasswordPage"),
			},
			{
