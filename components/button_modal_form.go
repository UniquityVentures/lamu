...
// "lamu-form-submit" from [getters.FormBubbling]. Only events whose form sits in
...
// the dialog is removed, "lamu:modal-closed" is dispatched, and the browser navigates.
...
func (e ButtonModalForm) Build(ctx context.Context) Node {
	...
	// Alpine @lamu-form-submit: POST the bubbling form via htmx.ajax, then close or swap the dialog.
  ...
  if (!m || m.id !== %s) return;
  evt.stopPropagation();
  if (f.dataset.lamuPostPending) return;
  f.dataset.lamuPostPending = '1';
  var u = (f.getAttribute('data-lamu-post-url')||'').trim();
  if (!u) { u = %s; }
  var body = document.body;
  function closeModal(x) {
    document.dispatchEvent(new CustomEvent('lamu:modal-closed', { bubbles: true, detail: Object.assign({ dialog: m }, x) }));
    m.remove();
  }
  var cleanup = function () {
    body.removeEventListener('htmx:beforeSwap', onBeforeSwap);
    delete f.dataset.lamuPostPending;
  };
  ...
  htmx.ajax('POST', u, {
	...
	return Div(Class("fk-modal-host"),
		Attr("@lamu-form-submit.window.stop", script),
